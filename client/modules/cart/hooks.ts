    ...
    async (
      ...
      try {
        ...
        if (res.error || !res.data || res.data.reserved !== true) {
          ...
          const existing = store.items.find((it) => it.productId === product.id && it.quantity === quantity)
          if (existing) {
            store.removeItem(existing.id)
          }

          const message = res.error ? res.error.message || "Failed to reserve stock" : "Unable to reserve stock"
