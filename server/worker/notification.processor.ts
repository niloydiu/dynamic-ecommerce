...
import { initializeDataSource, AppDataSource as _unused } from '../src/data-source';
...
async function run() {
  ...
  try {
    ...
    console.warn('Worker: Redis not available, exiting worker (no queue to process).');
    process.exit(0);
  }
  const worker = new Worker('notifications', async job => {
    if (job.name === 'product_restock') {
      ...
      const pending = await repo.find({ where: { product_id: productId, is_notified: false } });
      for (const req of pending) {
        // simulate email send
        console.log(`Worker: notifying ${req.email || req.user_id} about product ${productId}`);
