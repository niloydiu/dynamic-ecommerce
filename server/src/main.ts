  ...
  try {
      ...
      try {
        if (ds.options.type === 'postgres') {
          await ds.query(`CREATE INDEX IF NOT EXISTS idx_products_attributes ON products USING GIN (attributes jsonb_path_ops);`);
        }
      } catch (err) {
        console.warn('Could not create GIN index (may be using SQLite or Postgres unavailable).', err && err.message ? err.message : err);
      }
    ...
    try {
      ...
      registerImmediateHandler(async (name: string, data: any) => {
        if (name === 'product_restock') {
          ...
          const pending = await repo.find({ where: { product_id: data.productId, is_notified: false } });
          for (const req of pending) {
            console.log('Immediate handler: notifying', req.email || req.user_id, 'for product', data.productId);
      ...
      console.warn('Queue init failed or not present:', e && e.message ? e.message : e);
    }
