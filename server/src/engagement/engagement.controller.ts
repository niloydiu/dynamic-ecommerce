...
export class EngagementController {
  ...
  @Post('reaction')
  async reaction(@Body() body: { userId: string; productId: string; reaction: string }) {
    const { userId, productId, reaction } = body;
    return this.svc.addReaction(userId, productId, reaction);
  }

  @Post('comments')
  async comment(@Body() body: { userId: string; productId: string; text: string }) {
  ...
  @Post('stock-requests')
  async stockRequest(@Body() body: { userId?: string; email?: string; productId: string }) {
